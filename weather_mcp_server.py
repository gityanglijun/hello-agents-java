#!/usr/bin/env python3
"""天气查询 MCP 服务器 — 基于 hello_agents MCPServer"""
...
# 需要在 Python 路径中包含 HelloAgents 项目
# 如果导入失败，尝试自动添加路径
try:
    from hello_agents.protocols import MCPServer
except ImportError:
    # 自动查找 HelloAgents 项目路径
    script_dir = os.path.dirname(os.path.abspath(__file__))
    hello_agents_path = os.path.join(script_dir, "..", "..", "HelloAgents")
    if os.path.isdir(hello_agents_path):
        sys.path.insert(0, hello_agents_path)
        from hello_agents.protocols import MCPServer
    else:
        print(json.dumps({"error": "Cannot import hello_agents. Set PYTHONPATH to HelloAgents project."}))
        sys.exit(1)
