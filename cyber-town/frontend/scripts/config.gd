# 赛博小镇 - 全局配置
# ===============================
# autoload 机制：project.godot 中声明后，游戏启动时自动加载为单例。
# 任何脚本都可以直接用 Config.API_BASE_URL 访问，无需 preload()。
# ===============================
extends Node

# ==================== API配置 ====================
# Java 后端 (Javalin) 默认监听 8080 端口
const API_BASE_URL = "http://localhost:8080"
const API_CHAT = API_BASE_URL + "/chat"
const API_NPCS = API_BASE_URL + "/npcs"
const API_NPC_STATUS = API_BASE_URL + "/npcs/status"

# ==================== NPC配置 ====================
const NPC_NAMES = ["张三", "李四", "王五"]
const NPC_TITLES = {
	"张三": "Python工程师",
	"李四": "产品经理",
	"王五": "UI设计师"
}

# ==================== 游戏配置 ====================
const PLAYER_SPEED = 200.0
const INTERACTION_DISTANCE = 80.0
const NPC_STATUS_UPDATE_INTERVAL = 30.0  # 与后端 StateManager 保持一致

# ==================== UI配置 ====================
const DIALOGUE_FADE_TIME = 0.3
const NPC_LABEL_OFFSET = Vector2(0, -60)

# ==================== 调试配置 ====================
const DEBUG_MODE = true
const SHOW_INTERACTION_RANGE = true

# ==================== 工具函数 ====================
func log_info(message: String) -> void:
	if DEBUG_MODE:
		print("[INFO] ", message)

func log_error(message: String) -> void:
	print("[ERROR] ", message)

func log_api(endpoint: String, data: Dictionary) -> void:
	if DEBUG_MODE:
		print("[API] ", endpoint, " -> ", JSON.stringify(data))
